                          ...
                          org_eclipse_tahu_protobuf_Payload_Metric *metric)
{
    ...
    const size_t new_allocation_size =
        sizeof(org_eclipse_tahu_protobuf_Payload_Metric) * new_count;
    ...
    memcpy(&payload->metrics[old_count], metric,
           sizeof(org_eclipse_tahu_protobuf_Payload_Metric));
    return 0;
}

int set_propertyvalue(
    org_eclipse_tahu_protobuf_Payload_PropertyValue *propertyvalue,
    uint32_t datatype, const void *value, size_t size)
{
    ...
    switch (datatype)
    {
        ...
        case PROPERTY_DATA_TYPE_FLOAT:
            propertyvalue->which_value =
                org_eclipse_tahu_protobuf_Payload_PropertyValue_float_value_tag;
            propertyvalue->value.float_value = *(float *)value;
            break;
        case PROPERTY_DATA_TYPE_DOUBLE:
            propertyvalue->which_value =
                org_eclipse_tahu_protobuf_Payload_PropertyValue_double_value_tag;
            propertyvalue->value.double_value = *(double *)value;
            break;
        case PROPERTY_DATA_TYPE_BOOLEAN:
            propertyvalue->which_value =
                org_eclipse_tahu_protobuf_Payload_PropertyValue_boolean_value_tag;
        ...
        case PROPERTY_DATA_TYPE_TEXT:
            ...
            break;
        default:
            fprintf(stderr, "Invalid datatype(%lu) in set_propertyvalue\n",
                    datatype);
            return -1;
    }
    return 0;
...
int add_property_to_set(
    org_eclipse_tahu_protobuf_Payload_PropertySet *propertyset, const char *key,
    uint32_t datatype, const void *value, size_t size_of_value)
{
    ...
    const size_t value_allocation_size =
        sizeof(org_eclipse_tahu_protobuf_Payload_PropertyValue) * new_count;
    void *key_allocation_result =
        realloc(propertyset->keys, key_allocation_size);
    void *value_allocation_result =
        realloc(propertyset->values, value_allocation_size);
    // DEBUG_PRINT("key=%p value=%p\n", key_allocation_result,
    // value_allocation_result);
    ...
    memset(&propertyset->values[old_count], 0,
           sizeof(org_eclipse_tahu_protobuf_Payload_PropertyValue));
    ...
    {
        set_propertyvalue(&propertyset->values[old_count], datatype, value,
                          size_of_value);
    }
    return 0;
}

int add_propertyset_to_metric(
    org_eclipse_tahu_protobuf_Payload_Metric *metric,
...
int set_metric_value(org_eclipse_tahu_protobuf_Payload_Metric *metric,
                     uint32_t datatype, const void *value, size_t size)
{
    ...
    switch (datatype)
    {
        ...
        case METRIC_DATA_TYPE_DOUBLE:
            metric->which_value =
                org_eclipse_tahu_protobuf_Payload_Metric_double_value_tag;
        ...
        case METRIC_DATA_TYPE_UUID:
            metric->which_value =
                org_eclipse_tahu_protobuf_Payload_Metric_string_value_tag;
            metric->value.string_value = strndup(value, size);
            break;
        case METRIC_DATA_TYPE_DATASET:
            metric->which_value =
                org_eclipse_tahu_protobuf_Payload_Metric_dataset_value_tag;
            memcpy(&metric->value.dataset_value, value,
                   sizeof(metric->value.dataset_value));
            break;
        case METRIC_DATA_TYPE_TEMPLATE:
            metric->which_value =
                org_eclipse_tahu_protobuf_Payload_Metric_template_value_tag;
            memcpy(&metric->value.template_value, value,
                   sizeof(metric->value.template_value));
            break;
        case METRIC_DATA_TYPE_BYTES:
        case METRIC_DATA_TYPE_FILE:
        case METRIC_DATA_TYPE_UNKNOWN:
        default:
            fprintf(stderr, "Unhandled datatype(%lu) in set_metric_value\n",
                    datatype);
            return -1;
    }
    return 0;
...
int add_simple_metric(org_eclipse_tahu_protobuf_Payload *payload,
                      const char *name, bool has_alias, uint64_t alias,
                      uint64_t datatype, bool is_historical, bool is_transient,
                      const void *value, size_t size_of_value)
{
    // DEBUG_PRINT("Add simple metric...\n");
    // printf("=name: %s\n", name);
    static org_eclipse_tahu_protobuf_Payload_Metric new_metric;
    memset(&new_metric, 0, sizeof(new_metric));
    if (name != NULL)
    {
        new_metric.name = strdup(name);
        strcpy(new_metric.name, name);
...
ssize_t encode_payload(uint8_t *out_buffer, size_t buffer_length,
                       const org_eclipse_tahu_protobuf_Payload *payload)
{
    // Use a different stream if the user wants a normal encode or just a size
    // check
    pb_ostream_t sizing_stream = PB_OSTREAM_SIZING;
    pb_ostream_t buffer_stream =
        pb_ostream_from_buffer(out_buffer, buffer_length);
    pb_ostream_t *node_stream =
        ((out_buffer == NULL) ? &sizing_stream : &buffer_stream);

    // Encode the payload
    // DEBUG_PRINT("Encoding payload...\n");
    const bool encode_result = pb_encode(
        node_stream, org_eclipse_tahu_protobuf_Payload_fields, payload);
                       ...
                       const uint8_t *in_buffer, size_t buffer_length)
{
    ...
    const bool decode_result = pb_decode(
        &node_stream, org_eclipse_tahu_protobuf_Payload_fields, payload);
...
void reset_sparkplug_sequence(void) { payload_sequence = 0; }

int get_next_payload(org_eclipse_tahu_protobuf_Payload *payload)
{
    // Initialize payload
    // DEBUG_PRINT("Current Sequence Number: %lu\n", payload_sequence);
    memset(payload, 0, sizeof(org_eclipse_tahu_protobuf_Payload));
...
int init_dataset(org_eclipse_tahu_protobuf_Payload_DataSet *dataset,
                 uint64_t num_of_rows, uint64_t num_of_columns,
                 const uint32_t datatypes[], const char *column_keys[],
                 const org_eclipse_tahu_protobuf_Payload_DataSet_Row row_data[])
{
    ...
    {
        fprintf(stderr, "malloc(%zd) failure in init_dataset\n", key_size);
        return -1;
    }
    ...
    const size_t row_data_size =
        num_of_rows * sizeof(org_eclipse_tahu_protobuf_Payload_DataSet_Row);
...
#define EMPTY_PREFIX ""
void print_metadata(const char *prefix,
                    org_eclipse_tahu_protobuf_Payload_MetaData *metadata);
void print_propertyvalue(
    const char *prefix, org_eclipse_tahu_protobuf_Payload_PropertyValue *value);
void print_propertyset(
    const char *prefix,
    org_eclipse_tahu_protobuf_Payload_PropertySet *properties);
void print_propertysetlist(
    const char *prefix,
    org_eclipse_tahu_protobuf_Payload_PropertySetList *propertysetlist);
void print_dataset_row(const char *prefix,
                       org_eclipse_tahu_protobuf_Payload_DataSet_Row *row);
void print_datasetvalue(
    const char *prefix,
    org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue *dsvalue);
void print_dataset(const char *prefix,
                   org_eclipse_tahu_protobuf_Payload_DataSet *dataset_value);
void print_template_parameter(
    const char *prefix,
    org_eclipse_tahu_protobuf_Payload_Template_Parameter *template_parameter);
void print_template(const char *prefix,
                    org_eclipse_tahu_protobuf_Payload_Template *template);
void print_metric(const char *prefix,
                  org_eclipse_tahu_protobuf_Payload_Metric *metric);
                    ...
                    org_eclipse_tahu_protobuf_Payload_MetaData *metadata)
{
    ...
    //     PP("%sfile_name=%d [%p]\n", prefix, metadata->file_name,
    //     metadata->file_name);
    // }
    // if (metadata->file_type != NULL) {
    //     PP("%sfile_type=%s [%p]\n", prefix, metadata->file_type,
    //     metadata->file_type);
    // }
...
void print_propertyvalue(const char *prefix,
                         org_eclipse_tahu_protobuf_Payload_PropertyValue *value)
{
    // char temp[64];
    // if (prefix == NULL) {
    //     prefix = EMPTY_PREFIX;
    ...
    //     value->value.extension_value.extensions); break;
    // default:
    //     PP("%sinvalid which_value=%lu\n", prefix, value->which_value);
    // }
}
    ...
    org_eclipse_tahu_protobuf_Payload_PropertySet *properties)
{
    ...
    //     prefix = EMPTY_PREFIX;
    // }
    // PP("%skeys=[%p] (count=%lu)\n", prefix, properties->keys,
    // properties->keys_count); for (int i = 0; i < properties->keys_count; i++)
    // {
    //     PP("%s keys[%lu]=%s [%p]\n", prefix, i, properties->keys[i],
    //     properties->keys[i]);
    // }
    // PP("%svalues=[%p] (count=%lu)\n", prefix, properties->values,
    // properties->values_count); for (int i = 0; i < properties->values_count;
    // i++) {
    //     snprintf(temp, sizeof(temp), "%svalues[%lu].", prefix, i);
    ...
    org_eclipse_tahu_protobuf_Payload_PropertySetList *propertysetlist)
{
    ...
    // PP("%spropertyset=[%p] (count=%lu)\n", prefix,
    // propertysetlist->propertyset, propertysetlist->propertyset_count); for
    // (int i = 0; i < propertysetlist->propertyset_count; i++) {
    //     snprintf(temp, sizeof(temp), "%s.propertyset[%lu].", prefix, i);
    //     print_propertyset(temp, &propertysetlist->propertyset[i]);
    // }
    // if (propertysetlist->extensions != NULL) {
    //     PP("%sextensions=[%p] (display not supported)\n", prefix,
    //     propertysetlist->extensions);
    // }
}
                       ...
                       org_eclipse_tahu_protobuf_Payload_DataSet_Row *row)
{
    ...
    //     prefix = EMPTY_PREFIX;
    // }
    // PP("%selements=[%p] (count=%lu)\n", prefix, row->elements,
    // row->elements_count); for (int i = 0; i < row->elements_count; i++) {
    //     snprintf(temp, sizeof(temp), "%selements[%lu].", prefix, i);
    ...
    org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue *dsvalue)
{
    ...
    //     break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_long_value_tag:
    //     PP("%slong_value=%ld\n", prefix, dsvalue->value.long_value);
    //     break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_float_value_tag:
    //     PP("%sfloat_value=%f\n", prefix, dsvalue->value.float_value);
    //     break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_double_value_tag:
    //     PP("%sdouble_value=%f\n", prefix, dsvalue->value.double_value);
    //     break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_boolean_value_tag:
    //     PP("%sboolean_value=%lu\n", prefix, dsvalue->value.boolean_value);
    //     break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_string_value_tag:
    //     PP("%sstring_value=%s [%p]\n", prefix, dsvalue->value.string_value,
    //     dsvalue->value.string_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue_extension_value_tag:
    //     PP("%sextension_value=[%p] (display not supported)\n", prefix,
    //     dsvalue->value.extension_value.extensions); break;
    // default:
                   ...
                   org_eclipse_tahu_protobuf_Payload_DataSet *dataset_value)
{
    ...
    //     PP("%snum_of_columns=%zd\n", prefix, dataset_value->num_of_columns);
    // }
    // PP("%scolumns=[%p] (count=%lu)\n", prefix, dataset_value->columns,
    // dataset_value->columns_count); for (int i = 0; i <
    // dataset_value->columns_count; i++) {
    //     PP("%scolumn[%lu]=%s [%p]\n", prefix, i, dataset_value->columns[i],
    //     dataset_value->columns[i]);
    // }
    // PP("%stypes=[%p] (count=%lu)\n", prefix, dataset_value->types,
    // dataset_value->types_count); for (int i = 0; i <
    // dataset_value->types_count; i++) {
    //     PP("%stype[%lu]=%lu\n", prefix, i, dataset_value->types[i]);
    // }
    // PP("%srows=[%p] (count=%lu)\n", prefix, dataset_value->rows,
    // dataset_value->rows_count); for (int i = 0; i <
    // dataset_value->rows_count; i++) {
    //     snprintf(temp, sizeof(temp), "%srow[%lu].", prefix, i);
    ...
    org_eclipse_tahu_protobuf_Payload_Template_Parameter *template_parameter)
{
    ...
    //     PP("%sname=%s [%p]\n", prefix, template_parameter->name,
    ...
    //     PP("%slong_value=%ld\n", prefix,
    //     template_parameter->value.long_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_Template_Parameter_float_value_tag:
    //     PP("%sfloat_value=%f\n", prefix,
    //     template_parameter->value.float_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_Template_Parameter_double_value_tag:
    //     PP("%sdouble_value=%f\n", prefix,
    //     template_parameter->value.double_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_Template_Parameter_boolean_value_tag:
    //     PP("%sboolean_value=%lu\n", prefix,
    //     template_parameter->value.boolean_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_Template_Parameter_string_value_tag:
    //     PP("%sstring_value=%s [%p]\n", prefix,
    //     template_parameter->value.string_value,
    //     template_parameter->value.string_value); break;
    // case
    // org_eclipse_tahu_protobuf_Payload_Template_Parameter_extension_value_tag:
    //     PP("%sextension_value=[%p] (display not supported)\n", prefix,
    //     template_parameter->value.extension_value.extensions); break;
    // default:
    //     PP("%sinvalid which_value=%lu\n", prefix,
    //     template_parameter->which_value);
    // }
}
                    ...
                    org_eclipse_tahu_protobuf_Payload_Template *template)
{
    ...
    //     template->version);
    // }
    // PP("%smetrics=[%p] (count=%lu)\n", prefix, template->metrics,
    // template->metrics_count); for (int i = 0; i < template->metrics_count;
    // i++) {
    //     snprintf(temp, sizeof(temp), "%smetric[%lu].", prefix, i);
    //     print_metric(temp, &template->metrics[i]);
    // }
    // PP("%sparameters=[%p] (count=%lu)\n", prefix, template->parameters,
    // template->parameters_count); for (int i = 0; i <
    // template->parameters_count; i++) {
    //     snprintf(temp, sizeof(temp), "%sparameter[%lu].", prefix, i);
    //     print_template_parameter(temp, &template->parameters[i]);
    // }
    // if (template->template_ref != NULL) {
    //     PP("%stemplate_ref=%s [%p]\n", prefix, template->template_ref,
    //     template->template_ref);
    // }
    // if (template->has_is_definition) {
    //     PP("%sis_definition=%lu\n", prefix, template->is_definition);
    // }
                  ...
                  org_eclipse_tahu_protobuf_Payload_Metric *metric)
{
    ...
    switch (metric->which_value)
    {
        ...
        case org_eclipse_tahu_protobuf_Payload_Metric_bytes_value_tag:
            printf("%sbytes_value=[%p] (display not supported)\n", prefix,
                   metric->value.bytes_value);
            break;
        ...
        case org_eclipse_tahu_protobuf_Payload_Metric_extension_value_tag:
            printf("%sextension_value=[%p] (display not supported)\n", prefix,
                   metric->value.extension_value.extensions);
            break;
        default:
            printf("%sinvalid which_type=%lu", prefix, metric->which_value);
            break;
    }
}
...
void print_payload(org_eclipse_tahu_protobuf_Payload *payload)
{
    ...
    {
        printf("body=[%p] (display not supported)\n", payload->body);
    }
    if (payload->extensions != NULL)
    {
        printf("extensions=[%p] (display not supported)\n",
               payload->extensions);
    }
    printf("metrics=[%p] (count=%lu)\n", payload->metrics,
